...
class ProcessorTTXS(pepper.ProcessorTTbarLL):
    ...
    def process_selection(self, selector, dsname, is_mc, filler):
        ...
        #pos_triggers, neg_triggers = pepper.misc.get_trigger_paths_for_taozhe(
        #    dsname, is_mc, self.config["dataset_trigger_map"],
        #    self.config["dataset_trigger_order"], era=era)
        #if not is_mc:
        #  selector.add_cut("Trigger_Muon", partial(
        #    self.passing_neg_trigger, pos_triggers, neg_triggers))
        #selector.add_cut("Trigger", partial(
        #    self.passing_pos_and_neg_trigger, pos_triggers, neg_triggers))
        ...
        mu_trigs = [trig for trig in self.config["muon_triggers"] if trig in
                ak.fields(selector.final.HLT)]
        selector.set_column("muon_triggers", partial(
            self.passing_trigger, mu_trigs, []))

        #Numerator triggers
        pos_triggers, neg_triggers = pepper.misc.get_trigger_paths_for(
            dsname, is_mc, self.config["dataset_trigger_map"],
            self.config["dataset_trigger_order"], era=era)
        selector.set_column("jet_triggers", partial(
    ...
    def process_selection_jet_part(self, selector, is_mc, variation, dsname,
        ...
        #selector.add_cut("Has_at_least_two_fatjets", self.At_least_two_fatjets)
        #selector.add_cut("Has_at_least_one_jet", self.At_least_one_jet)
    ...
    def At_least_one_jet(self, data):
        """select events that contain at least 1 reco level jets"""
        accept = np.asarray(ak.num(data["Jet"]) >= 1) 
        return accept
