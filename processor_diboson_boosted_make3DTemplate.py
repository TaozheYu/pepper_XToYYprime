...
class ProcessorTTXS(pepper.ProcessorTTbarLL):
    ...
    def process_selection_jet_part(self, selector, is_mc, variation, dsname,
        ...
        selector.set_column("FatJet_softdrop_mass", self.FatJet_softdrop_mass)
    ...
    def At_least_two_fatjets(self, data):
        """select events that contain at least 1 reco level fatjets"""
        #breakpoint()
        accept = np.asarray(ak.num(data["FatJet"]) >= 2) 
        return accept
    ...
    def Reco_Y_mass(self, data):
        ...
        return Y_mass     
        #Yprime_mass = (data["FatJet"][:, 0]).mass
    '''
    def Reco_Yprime_mass(self, data):
        ...
        return Yprime_mass
        '''
    def FatJet_softdrop_mass(self, data):
        return (data["FatJet"][:, 0]).msoftdrop
    ...
    def Reco_Yprime_mass(self, data):
        reco_fatjet = ak.pad_none(data["FatJet"], 2, axis=1)
        abs_diff_from_Y_col1 = np.abs(reco_fatjet[:, 0].mass - 500)
        abs_diff_from_Y_col2 = np.abs(reco_fatjet[:, 1].mass - 500)

        Yprime_mass = np.where(abs_diff_from_Y_col1 < abs_diff_from_Y_col2, reco_fatjet[:, 0].mass, reco_fatjet[:, 1].mass)
        
        return Yprime_mass     
        #Yprime_mass = (data["FatJet"][:, 0]).mass
    '''
    def Reco_X_mass(self, dsname, data):
        #if dsname.startswith("XToYYprime_MX1000") | dsname.startswith("QCD") | dsname.startswith("TT"):
          jet1_eta, jet1_phi = data["Jet"][:,0].eta, data["Jet"][:,0].phi
          jet2_eta, jet2_phi = data["Jet"][:,1].eta, data["Jet"][:,1].phi
        ...
        #if dsname.startswith("XToYYprime_MX3000"):
        #  reco_fatjet = ak.pad_none(data["FatJet"], 2, axis=1)
        #  X_mass = np.where( ak.num(data["FatJet"]) >=2, (reco_fatjet[:,0] + reco_fatjet[:,1]).mass, -99)
        #  return X_mass
    '''
    ...
    def Reco_X_mass(self, dsname, data):
        ...
        fatjet1 = data["FatJet"][:, 0]
        fatjet1_p4 = vector.awk( 
          ak.zip({
          "pt": fatjet1.pt,
          "eta": fatjet1.eta,
          "phi": fatjet1.phi,
          "mass": fatjet1.msoftdrop
          }, with_name="Momentum4D")
        )
        fatjet2 = data["FatJet"][:, 1]
        fatjet2_p4 = vector.awk(
          ak.zip({
          "pt": fatjet2.pt,
          "eta": fatjet2.eta,
          "phi": fatjet2.phi,
          "mass": fatjet2.msoftdrop
          }, with_name="Momentum4D")
        )
        # 计算每个jet与leading fatjet的deltaR
        #delta_eta = jets_p4.eta - ak.broadcast_arrays(fatjet_p4.eta, jets_p4.eta)[0]
        ...
        # 将有效值填充到输出数组中
        valid_mass = (fatjet1_p4 + fatjet2_p4).mass
        invariant_mass = ak.to_numpy(valid_mass)  # 转换为 NumPy 并填充
