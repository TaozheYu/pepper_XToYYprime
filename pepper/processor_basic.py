...
class ProcessorBasicPhysics(pepper.Processor):
    ...
    def get_jetmet_variation_args(self):
        ...
        if "jet_resolution" in self.config and "jet_ressf" in self.config:
            ret.append(VariationArg("JerUp", jer="up"))
            ret.append(VariationArg("JerDown", jer="down"))
        return ret
    ...
    def add_generator_uncertainies(self, dsname, selector):
        ...
        if ("QCD_madgraph_herwig7" not in dsname) and ("QCD_pythia8_Pt" not in dsname) and ("QCD_herwig7_Pt" not in dsname) and ("Dijet_NNPDF30_powheg_pythia8" not in dsname):
    ...
    def passing_neg_trigger(self, pos_triggers, neg_triggers, data):
        """Return mask for events that pass the trigger."""
        hlt = data["HLT"]
        available = ak.fields(hlt)
        triggered = np.full(len(data), False)
        triggered_pos = np.full(len(data), False)
        if not neg_triggers:
           triggered_neg = np.full(len(data), True)
        else:
           triggered_neg = np.full(len(data), False)
           print (neg_triggers)
           print (pos_triggers)
        for trigger_path in pos_triggers:
            if trigger_path not in available:
                logger.debug(f"HLT_{trigger_path} not in file")
                continue
            triggered_pos |= np.asarray(hlt[trigger_path])
        for trigger_path in neg_triggers:
            if trigger_path not in available:
                continue
            triggered_neg |= np.asarray(hlt[trigger_path])

        triggered = (triggered_neg)
        return triggered

    def passing_pos_trigger(self, pos_triggers, neg_triggers, data):
        """Return mask for events that pass the trigger."""
        hlt = data["HLT"]
        available = ak.fields(hlt)
        triggered = np.full(len(data), False)
        ...
        triggered = (triggered_pos)
        return triggered

    def passing_pos_and_neg_trigger(self, pos_triggers, neg_triggers, data):
        """Return mask for events that pass the trigger."""
        hlt = data["HLT"]
        available = ak.fields(hlt)
        triggered = np.full(len(data), False)
    ...
    def build_fatjet_column(self, is_mc, data):
        jet = data["FatJet"]
        lepton = data["Lepton"]
        # Remove large-R jet built from a prompt-lepton, useful for 1L+ channels
        has_lepton_close = ak.any(jet.metric_table(lepton) < 0.8, axis=2)
        
        # id. Data["FatJet_FatJet_jetId"] == 6 (tightlepveto == true)
        has_id = True
        has_id = jet.isTightLeptonVeto

        is_good = ( #replicate the syntax in pepper/processor_basic.py
            has_id
            & (~has_lepton_close)
            & (abs(jet.eta) < self.config["good_fatjet_eta_max"])
            & (jet.pt > self.config["good_fatjet_pt_min"]))
    
        jet = jet[ak.argsort(jet["pt"], ascending=False)]

        if is_mc: #is_mc == True
          has_gen_close = ak.fill_none(jet.delta_r(jet.matched_gen) < 0.8, False) #if no matched gen-jet, replace None with False
          #had_top = self.hadtop(data) #find hadronic top-quarks
          #gen_top_matched = ak.any(jet.matched_gen.metric_table(had_top) < 0.8, axis=2) #gen-jet matched to a hadronic-top
          #jet["matched_hadtop"] = has_gen_close & gen_top_matched
        # return jets passing selection
        return jet[is_good]
